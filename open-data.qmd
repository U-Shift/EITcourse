...
You can access it's visualization tool at [www.openstreetmap.org](https://www.openstreetmap.org/). To edit the map, you can use the [Editor](https://www.openstreetmap.org/edit), once you register.
...
Access via [export.hotosm.org](https://export.hotosm.org/). You need an OSM account to use it.
...
General Transit Feed Specification (GTFS) is standardized format for documenting public transportation dataset, including routes, schedule, stop locations, and trips. Transit agencies are responsible for maintaining the data up to date. The data is given in .txt files and are related to one another by "id".

GTFS data can be downloaded in the following websites:
...
The European Union has a directive that requires the member states to provide access to transportation data. Data includes not only **Public Transportation** data, but also **road network**s, car **parking**, and other transportation-related information.
